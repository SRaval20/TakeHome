...
export default function ProfileButton() {
    ...
    useEffect(() => {
      if (session?.user) {
        const fetchData = async () => {
          ...
          setUser(fetchedUser);
          const userBalance = await getUserShopBalance(fetchedUser.username);
          setShopBalance(userBalance);
        };
        fetchData();
      }
    }, [session?.user]);    
  ...
  */
  ...
  if (status === "loading" || !user) {
    // If the user is loading or not logged in, show loading UI
    return (
      <Button className="font-mono bg-[#194D47] hover:bg-[#2D6E56]">
        <Loader2 className="animate-spin" />
      </Button>
  ...
  if (!session) {
    // If the user is not authenticated, show login/signup UI
    return (
      <div>
        <Button
          className="bg-[#194D47] hover:bg-[#2D6E56]"
  ...
  // If the user is authenticated, show the profile button with dropdown menu
  return (
    <div>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
